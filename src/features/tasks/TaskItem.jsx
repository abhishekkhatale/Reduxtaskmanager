    ...
    <div className="flex items-center justify-between p-3 border-b border-gray-200">
      <div className="flex items-center gap-3 flex-1">
        <button
          ...
          className={`p-2 rounded-full ${task.completed ? 'bg-gray-800 text-white' : 'border border-gray-300'}`}
        ...
        <div className="flex-1 min-w-0">
          ...
          {task.description && (
            <p className={`text-xs truncate ${task.completed ? 'text-gray-400' : 'text-gray-600'}`}>
              {task.description}
            </p>
          )}
          {task.time && (
